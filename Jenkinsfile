node {
    ...
    stage('Build image') {
        ...
        app = docker.build("node-version-pi")
    }
    ...
    stage('Push image') {
        ...
        */
        sh "docker rmi -f ${ART_DOCKER_REGISTRY}/node-version-pi:${env.BUILD_NUMBER}"
        sh "docker rmi -f ${ART_DOCKER_REGISTRY}/node-version-pi:latest"
